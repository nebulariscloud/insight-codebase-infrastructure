...
variable "account_id" {
  description = <<-EOT
    Audit account's 12-digit AWS account ID. If empty, the provider falls back
...
variable "account_id_ssm_path" {
  description = "SSM path in SharedServices that holds the Audit account ID. Only used when account_id is empty."
  type        = string
  default     = "/accelerator/organization/account-ids/Audit"
}
