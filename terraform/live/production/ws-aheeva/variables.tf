...
variable "ftps_client_cidrs" {
  description = <<-EOT
    Source CIDRs of the FTPS file-drop clients, allowed on the control +
    passive ports. From the source SG (sg-0236c297e78a62ab2). Confirm/trim the
    real client list and coordinate the new endpoint with each before cutover.
  EOT
  type        = list(string)
  default     = []
}
