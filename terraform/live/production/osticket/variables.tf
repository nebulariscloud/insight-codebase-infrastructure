...
variable "ami_id" {
  description = <<-EOT
    AMI ID in us-east-2, registered from the Lightsail `osticket1` snapshot
    export chain. The Lightsail export lands encrypted with the AWS-managed
    `aws/ebs` key, which cannot be shared cross-account, so it goes via a
    transfer CMK and is finally re-encrypted with the LZA EBS key. See the
    provenance diagram in README.md.
  EOT
  type        = string
