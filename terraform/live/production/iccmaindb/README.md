...
```sql
-- On the source: create a replication user (once).
CALL mysql.rds_set_configuration('binlog retention hours', 168);
-- On the destination: point at the source and start.
CALL mysql.rds_set_external_master(
  'iccmaindb.cqmvz00gvdb5.us-east-1.rds.amazonaws.com', 3306,
  'repl_user', 'repl_password', '<binlog_file>', <binlog_pos>, 0);
CALL mysql.rds_start_replication;
SHOW SLAVE STATUS\G   -- watch Seconds_Behind_Master -> 0
```

The `<binlog_file>` / `<binlog_pos>` come from the source snapshot's creation
point (the RDS event log / `SHOW MASTER STATUS` captured at snapshot time).
