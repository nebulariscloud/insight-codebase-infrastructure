...
variable "f9_bucket_name" {
  description = <<-EOT
    Name of the f9-recordings S3 bucket the instance needs access to.
    Defaults to "f9-recordings-prod-<account_id>", matching the default
    in terraform/live/production/f9-recordings/. Override only if the
...
variable "f9_bucket_kms_key_arn" {
  description = <<-EOT
    ARN of the customer-managed KMS key encrypting the f9-recordings
    ...
    Get the value with:
      ...
      aws s3api get-bucket-encryption \
        --bucket f9-recordings-prod-<account_id> --region us-east-2 \
