...
#
# Same pattern as the existing `sftp-server` and `sftp-server-claro` leaves:
# a private-only EC2 in shared-prod, fronted by an internet-facing NLB in
# the Perimeter ingress VPC over TGW. Sibling leaf is
# `terraform/live/perimeter/sftp-f9-nlb/`.
#
# AMI provenance:
#   Source (us-east-1): ami-0207938d1f8eedf49 / snap-0a9a4f2996e6cbd07
#                       from instance i-018f8ea99b6beafab, account 254422596287.
#                       Source snapshot was UNENCRYPTED, so no transfer CMK
#                       was needed - the AMI and snapshot were shared directly.
#   Target (us-east-2): ami-0545c53be16039a74 / snap-01b7371428ef4d68f
#                       Re-encrypted with alias/accelerator/ebs/default-encryption/key
#                       Root device /dev/xvda, 20 GiB, gp3.
#                       ProductCodes empty - verified before use, because a
#                       marketplace product code is the one failure no retry
#                       fixes (it killed the CTI v7 apply on 2026-07-05).
#
...
module "ec2_migrated" {
  ...
  # access to the f9-recordings bucket in the sibling leaf.
  ...
  # Attach the data volume only when a snapshot was provided. The F9 AMI is
  # root-only - confirmed against the source instance i-018f8ea99b6beafab,
  # which had exactly one attachment (vol-034113a44fc62ac97 at /dev/xvda) -
  # so data_volume_snapshot_id stays empty and this list collapses.
...
# pattern used by sftp-server / sftp-server-claro / wazuh-nlb / wazuh-ga.)
###############################################################################
...
output "private_ip" {
  description = "Private IP. Paste into terraform/live/perimeter/sftp-f9-nlb/terraform.tfvars as sftp_server_private_ip."
