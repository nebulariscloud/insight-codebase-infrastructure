...
variable "amex_bucket_name" {
  description = <<-EOT
    Name of the amex-recordings S3 bucket the instance needs access to.
    Defaults to "amex-recordings-prod-<account_id>", matching the default
    in terraform/live/production/amex-recordings/. Override only if the
...
variable "amex_bucket_kms_key_arn" {
  description = <<-EOT
    ARN of the customer-managed KMS key encrypting the amex-recordings
    bucket. Required when the bucket uses SSE-KMS (the default in this
    environment). Without it, server-side KMS calls during PutObject /
    GetObject get denied and s3fs returns EPERM at close time.

    Get the value with:

      aws s3api get-bucket-encryption \
        --bucket amex-recordings-prod-<account_id> --region us-east-2 \
