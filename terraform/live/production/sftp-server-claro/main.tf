...
#
# Same pattern as the existing `sftp-server` leaf: a private-only EC2 in
# shared-prod, fronted by an internet-facing NLB in the Perimeter ingress
# VPC over TGW. Sibling leaf is `terraform/live/perimeter/sftp-claro-nlb/`.
#
# AMI provenance:
#   Source (us-east-1): ami-08a70ee672a3be576 / snap-0f0ec998581c40ad1
#   Target (us-east-2): ami-02720404eb5b85c63 / snap-04370f749b46cef5c
#                       Re-encrypted with alias/accelerator/ebs/default-encryption/key
#                       Root device /dev/sda1, 20 GiB, gp3.
#
...
# pattern used by sftp-server / wazuh-nlb / wazuh-ga.)
###############################################################################
...
output "private_ip" {
  description = "Private IP. Paste into terraform/live/perimeter/sftp-claro-nlb/terraform.tfvars as sftp_server_private_ip."
