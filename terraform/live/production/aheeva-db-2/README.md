# Aheeva DB 2 (Production / shared-prod) — private
...
| Fact | Value |
|---|---|
| Source instance | `i-097ec187250e48e21`, `m5.2xlarge` |
| Root volume | `vol-0eeebcba000a473cc`, 200 GiB gp2, **unencrypted** |
| Source AMI | `ami-011939b19c6bd1492` (shared with the Asterisk + DB 1 boxes) |
...
```bash
aws ec2 create-image --region us-east-1 --instance-id i-097ec187250e48e21 \
  --name "aheeva-db-2-migration-$(date +%Y%m%d)" --no-reboot
...
- `terraform/live/production/aheeva-asterisk-1/`, `aheeva-db-1/` — sibling cluster boxes
