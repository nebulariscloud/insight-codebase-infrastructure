# Amex recordings bucket (Production)

S3 bucket for recording storage in the Production spoke. Object Lock is
enabled at create time, versioning is on, and the bucket is locked down
to TLS-only with all public access blocked.

## What this owns

- One S3 bucket: `amex-recordings-prod-<account_id>` (overridable).
...
- IAM policies for producers / consumers - grant via separate roles.
...
```bash
cd terraform/live/production/amex-recordings
