...
preserve_client_ip makes the NLB SNAT, so the reply path stays
symmetric.
...
alternative is adding explicit shared-prod route table entries that
return the agent traffic via TGW to Perimeter — fragile because public
IPs are unbounded.
