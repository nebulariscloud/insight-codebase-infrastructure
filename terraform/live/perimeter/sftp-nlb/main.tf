###############################################################################
# SFTP NLB (Perimeter ingress)
#
# Internet-facing Network Load Balancer that fronts the SFTP server living
# privately in shared-prod (Production account). The NLB has a single TCP
# listener on the SFTP port forwarding to the SFTP server's private IP via
# IP target. Cross-VPC reachability is provided by the existing TGW that
# already connects perimeter <-> shared-prod (used by the wazuh-nlb stack
# the same way).
#
# Static IPs:
...
#   destroyed and recreated (which Terraform won't do unless you remove
#   deletion_protection and force replacement). The two IPs are exposed
#   via the `nlb_public_ips` output for partner allowlisting.
#
# Source IPs visible to the SFTP server:
#   preserve_client_ip = false (matches wazuh-nlb's documented decision).
...
#   the NLB SG layer (allowed_source_cidrs), not on the SFTP server. If
#   real client IPs become a hard requirement, the alternative is
#   reworking shared-prod's return-path routing - see the long comment on
#   wazuh-nlb's agent_event target group for the asymmetric-routing
#   reason this is off.
###############################################################################
...
resource "aws_security_group" "nlb" {
  ...
  description = "Security group for the SFTP NLB"
...
resource "aws_lb_target_group" "sftp" {
  ...
  # Off on purpose; see the long comment on wazuh-nlb's agent_event tg.
  # Same return-path issue applies: the SFTP server's reply to a public
  # client would have to leave shared-prod via its 0.0.0.0/0 default
  # route through the egress/inspection VPC, breaking symmetry. With
  # SNAT on, the reply stays in-VPC perimeter <-> shared-prod over TGW.
...
output "nlb_public_ips" {
  description = <<-EOT
    ...
    to partners for allowlisting along with the DNS name.
  EOT
  value = sort([
