###############################################################################
# SFTP NLB - Claro (Perimeter ingress)
#
# Internet-facing Network Load Balancer that fronts sftp-server-claro
# (private in shared-prod / Production). Same shape as the existing
# sftp-nlb leaf - one TCP listener forwarding to the server's private IP
# via IP target. Cross-VPC reachability is provided by the existing TGW
# that already connects perimeter <-> shared-prod (used by the sibling
# sftp-nlb / wazuh-nlb stacks the same way).
#
# Static IPs:
...
resource "aws_security_group" "nlb" {
  ...
  description = "Security group for the Claro SFTP NLB"
...
output "nlb_public_ips" {
  description = <<-EOT
    ...
    to Claro for allowlisting along with the DNS name.
  EOT
  value = sort([
