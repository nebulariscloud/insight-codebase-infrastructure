...
| 9 | `S3.22` | S3 object-level write events in CloudTrail | `Manual` | one-time Console change on Control Tower `BaselineCloudTrail` in Management account; documented in `cloudtrail-data-events-runbook.md` (D-9 resolved option c) — **Wave 1 complete via runbook execution** | Org-wide | 2/3/3 | 15 | 1 |
| 10 | `S3.23` | S3 object-level read events in CloudTrail | `Manual` | same runbook as #9, ReadOnly selector — **Wave 1 complete via runbook execution** | Org-wide | 2/3/3 | 15 | 1 |
| 11 | `S3.5` | S3 buckets require SSL | `Config-Rule-SSM` | Config rule `s3-bucket-ssl-requests-only` + SSM `AWSConfigRemediation-RestrictBucketSSLRequestsOnly` — **Wave 1 complete 2026-06-17** via spec `lza-config-rules-and-ssm-remediations` | All buckets (findings 11, 27, 40, 46) | 2/3/3 | 15 | 1 |
| 12 | `S3.17` | S3 buckets encrypted with KMS | `Config-Rule-SSM` | Config rule `s3-default-encryption-kms` + custom SSM `Accelerator-SSM-Enable-S3-Bucket-KMS-Encryption` reading `alias/accelerator/s3-default` CMK from SSM Parameter Store at `/accelerator/kms/AcceleratorS3DefaultKey/key-arn` — **Wave 1 complete 2026-06-17** | All buckets (findings 14, 42) | 2/3/3 | 15 | 1 |
| 13 | `S3.9` | S3 server access logging | `Config-Rule-SSM` | Config rule `s3-bucket-logging-enabled` + custom SSM `Accelerator-SSM-Enable-S3-Bucket-Logging` (param: per-region access-log bucket via `${ACCEL_LOOKUP::Bucket:s3AccessLogs}`) — **Wave 1 complete 2026-06-17** | All buckets (findings 15, 30, 43) | 2/3/3 | 15 | 1 |
| 14 | `SNS.1` | SNS topic encryption with KMS | `Config-Rule-SSM` | Config rule `sns-encrypted-kms` + custom SSM `Accelerator-SSM-Enable-SNS-KMS-Encryption` (uses `alias/aws/sns` per design Decision 9). Doc skips `aws-controltower-*` topics (SCP `p-ijjfwspk` denies modification). 3 PCI Control-Tower-owned topic findings explicitly **SUPPRESSED** in Security Hub on 2026-06-18 with audit-trail note. **Wave 1 complete 2026-06-18** | All topics, org-wide | 2/3/3 | 15 | 1 |
| 15 | `S3.10` | S3 buckets with versioning have lifecycle | `Config-Rule-SSM` | covered by S3.13 rule (single Config rule `s3-lifecycle-policy-check` satisfies both, per spec design Decision 2) — **Wave 1 complete 2026-06-17** | All versioned buckets | 2/2/3 | 13 | 1 |
...
| 17 | `CloudWatch.16` | Log group retention configured | `LZA-Config` + `Config-Rule-SSM` | `global-config.yaml` already sets `cloudwatchLogRetentionInDays: 365` for LZA-created groups; Config rule `cw-loggroup-retention-period-check` + custom SSM `Accelerator-SSM-Set-CloudWatch-Log-Group-Retention` covers non-LZA groups. Doc skips `aws-controltower-*` and `/aws/lambda/aws-controltower-*` log groups (SCP `p-ijjfwspk` Sid `GRCWLG` denies `PutRetentionPolicy`) and gracefully handles AccessDeniedException for any other org-protected group. 3 PCI Control-Tower-owned log group findings explicitly **SUPPRESSED** in Security Hub on 2026-06-18 with audit-trail note. **Wave 1 complete 2026-06-18** | All log groups | 2/3/3 | 15 | 1 |
...
| 35 | `S3.13` | S3 lifecycle configurations | `Config-Rule-SSM` | Config rule `s3-lifecycle-policy-check` + custom SSM `Accelerator-SSM-Apply-Default-S3-Lifecycle` (transitions only, no expirations: 90d→IA, 180d→Glacier, 365d→DeepArchive on current; 30d/90d/180d on noncurrent; tag-opt-out via `accelerator:s3-lifecycle-managed=false`) — **Wave 1 complete 2026-06-17** | All buckets (findings 53, 56) | 1/3/3 | 12 | 1 |
